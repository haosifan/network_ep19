...
edge_list <- allfriends %>% 
  filter((user %in% meps_list_withtwitter$twitter_id) | (user_id %in% meps_list_withtwitter$twitter_id))
...
```{r}
meps_list_withtwitter %>% filter(!(twitter_id %in% edge_list$user_id))

get_friends("rovanaplumb") %>% filter(user_id %in% meps_list_withtwitter$twitter_id)
